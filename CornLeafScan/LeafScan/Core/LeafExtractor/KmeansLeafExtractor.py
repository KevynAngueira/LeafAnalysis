...
class KmeansLeafExtractor:
    def __init__(self, config: LeafExtractorConfig=None, clusters = 7, padding = 20, down_factor=4):
        if config is None:
    ...
    def _imagePreprocessing(self, image):
        ...
        #with timer("First LAB Init"):
        temp = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
        
        #with timer("LAB Big"):
        lab_big = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
       
        #with timer("LAB Small"):
        h, w = image.shape[:2]
        w_small, h_small = w // self.down_factor, h // self.down_factor
        img_small = cv2.resize(image, (w_small, h_small))
        lab_small = cv2.cvtColor(img_small, cv2.COLOR_BGR2LAB)

        #with timer("Masking"):
        lower_bound, upper_bound = self.leaf_bounds
        mask_small = cv2.inRange(lab_small, lower_bound, upper_bound)
        leaf_mask = cv2.resize(mask_small, (w, h), interpolation=cv2.INTER_NEAREST)
            
        #with timer("Anchor Points"):
        anchored_image, anchored_mask = self._addAnchorPoints(image, leaf_mask)

        #with timer("Convert to Lab"):
