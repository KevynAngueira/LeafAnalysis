...
from LeafScan.Utils import resize_for_display, cropAndRotate

from LeafScan.Configs import ViewExtractorConfig

from .ViewExtractor import ViewExtractor

class StabilizedViewExtractor(ViewExtractor):
    def __init__(self, view_config: ViewExtractorConfig = None, movement_threshold=5, alpha: float = 0.1, confirmation_frames=5):
    ...
    def __handleLostTarget(self):
        ...
        is_lost = self.lost_confirmation_counter >= self.confirmation_frames
        if is_lost:
            # Confirmed target is lost, set view_window to empty
            print(f"!!! View Window is Lost, results may be innacurate. !!!")
    ...
    def Extract(self, image, display=False, stabilize=True):
        ...
        if target_box is not None:
            ...
            self.accepted_rect = stabilized_rect
            self.lost_confirmation_counter = 0
        else:
            self.__handleLostTarget()

        view_window = cropAndRotate(image, self.accepted_rect)
